...
namespace FriishProduce.Injectors
{
    class NeoGeo
    {
        ...
        {
            if (File.Exists(Paths.WorkingFolder_Content5 + "banner.bin"))
            {
                byte[] content1 = File.ReadAllBytes(Paths.WorkingFolder_Content5 + "banner.bin");
                var tpl = new List<byte>();
                for (int i = 0xA00; i < 1000; i++) // Dummy number
                    tpl.Add(content1[i]);
        ...
        internal void InsertSaveTitle(string title, string tpl = "")
        {
            if (File.Exists(Paths.WorkingFolder_Content5 + "banner.bin"))
            {
                // TO-DO
